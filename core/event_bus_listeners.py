...
from models.events import PlayerMoved, PlayerRollDice
from models.commands import MovePlayer
...
async def handle_player_landing(event: PlayerMoved):
    game_state = state_manager.get_state(event.game_id)
    ...
    if isinstance(landed_space, PropertySpace):
        ...
        if landed_space.owned_by == user_state.user_id:
            ...
            await send_wsp_event(ws, WSPEvent(
                event="LandedOnSelfOwnedProperty",
        ...
        await send_wsp_event(ws, WSPEvent(
            event="LandedOnOpponentProperty",
            data={"space": landed_space.model_dump()}
        ))
        return
